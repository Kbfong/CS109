...
void window::keyboard (GLubyte key, int x, int y) {
   ...
   switch (key) {
      case 'Q': case 'q': case ESC:
         ...
         break;
      case 'H': case 'h':
         move_selected_object(-1*moveAmount,0);
         break;
      case 'J': case 'j':
         move_selected_object(0, -1*moveAmount);
         break;
      case 'K': case 'k':
         move_selected_object(0, 1*moveAmount);
         break;
...
void window::special (int key, int x, int y) {
   ...
   window::mus.set (x, y);
   switch (key) {
      case GLUT_KEY_LEFT: move_selected_object (-1*moveAmount, 0); 
      break;
      case GLUT_KEY_DOWN: move_selected_object (0, -1*moveAmount); 
      break;
      case GLUT_KEY_UP: move_selected_object (0, 1*moveAmount); 
      break;
      case GLUT_KEY_RIGHT: move_selected_object (1*moveAmount, 0); 
      break;
